...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, config: dict = None) -> None:
        ...
        self.ends = [100, 100, 100, 100]  # Before, converging, merge, after
    ...
    def reset(self, is_training=True, testing_seeds=0) -> Observation:
        ...
        self.vehicle_speed = []
        self.vehicle_pos = []
        self._reset(num_CAV=self.num_CAV)
    ...
    def step(self, action: Action, env) -> Tuple[Observation, float, bool, dict]:
        """
        ...
        obs = self.observation_type.observe()
        reward = self._reward(action, obs ,env)
        ...
        for v in self.road.vehicles:
            if str(v)[:3]== 'MDP' and v.crashed:
                cav_crashed = True

        info = {
            "speed": self.vehicle.speed,
            "cav_crashed": cav_crashed,
            # "action": action,
